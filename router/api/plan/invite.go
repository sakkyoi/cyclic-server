...
func (*Plan) Invite(c *gin.Context) {
	...
	if err != nil {
		c.JSON(http.StatusBadRequest, model.ErrorResponse{Type: model.ErrorInvalidInput, Error: "invalid user id", Detail: err.Error()})
		return
	}

	// check if the user is the host
	if hostID == userID {
		c.JSON(http.StatusBadRequest, model.ErrorResponse{Type: model.ErrorInvalidInput, Error: "cannot invite yourself"})
		return
	}
	...
	if ent.IsNotFound(err) {
		...
		c.JSON(http.StatusInternalServerError, model.ErrorResponse{Type: model.ErrorInternal, Error: "failed to get plan", Detail: err.Error()})
		return
	}
	...
	u, err := secretary.Minute.User.Query().
		...
		Only(c)
	if ent.IsNotFound(err) {
		c.JSON(http.StatusNotFound, model.ErrorResponse{Type: model.ErrorUserNotFound, Error: "user not found"})
		return
	} else if err != nil {
		c.JSON(http.StatusInternalServerError, model.ErrorResponse{Type: model.ErrorInternal, Error: "failed to get user", Detail: err.Error()})
		return
	}
	...
	if ent.IsConstraintError(err) {
		c.JSON(http.StatusBadRequest, model.ErrorResponse{Type: model.ErrorSubscriptionExists, Error: "user already invited to the plan"})
		return
	} else if err != nil {
		c.JSON(http.StatusInternalServerError, model.ErrorResponse{Type: model.ErrorInternal, Error: "failed to invite user to the plan", Detail: err.Error()})
		return
	}
