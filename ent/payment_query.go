...
func (pq *PaymentQuery) QueryRecords() *RecordQuery {
	query := (&RecordClient{config: pq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := pq.prepareQuery(ctx); err != nil {
			return nil, err
		}
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.O2O, false, payment.RecordsTable, payment.RecordsColumn),
		)
		fromU = sqlgraph.SetNeighbors(pq.driver.Dialect(), step)
		return fromU, nil
	}
...
func (pq *PaymentQuery) Clone() *PaymentQuery {
	...
	return &PaymentQuery{
		...
		withRecords: pq.withRecords.Clone(),
...
func (pq *PaymentQuery) WithRecords(opts ...func(*RecordQuery)) *PaymentQuery {
	query := (&RecordClient{config: pq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	pq.withRecords = query
	return pq
}
...
func (pq *PaymentQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Payment, error) {
	...
	if query := pq.withRecords; query != nil {
		if err := pq.loadRecords(ctx, query, nodes, nil,
			func(n *Payment, e *Record) { n.Edges.Records = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (pq *PaymentQuery) loadRecords(ctx context.Context, query *RecordQuery, nodes []*Payment, init func(*Payment), assign func(*Payment, *Record)) error {
	...
	query.Where(predicate.Record(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(payment.RecordsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.payment_records
		if fk == nil {
			return fmt.Errorf(`foreign-key "payment_records" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "payment_records" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
