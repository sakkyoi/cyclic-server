...
func (rq *RecordQuery) QuerySubscription() *SubscriptionQuery {
	query := (&SubscriptionClient{config: rq.config}).Query()
...
func (rq *RecordQuery) WithSubscription(opts ...func(*SubscriptionQuery)) *RecordQuery {
	query := (&SubscriptionClient{config: rq.config}).Query()
...
//
//	client.Record.Query().
//		GroupBy(record.FieldDeclareFor).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
func (rq *RecordQuery) loadSubscription(ctx context.Context, query *SubscriptionQuery, nodes []*Record, init func(*Record), assign func(*Record, *Subscription)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Record)
	for i := range nodes {
		if nodes[i].subscription_records == nil {
			continue
		}
		fk := *nodes[i].subscription_records
