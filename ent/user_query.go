...
func (uq *UserQuery) loadPlans(ctx context.Context, query *PlanQuery, nodes []*User, init func(*User), assign func(*User, *Plan)) error {
	...
	query.Where(predicate.Plan(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.PlansColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		...
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "user_plans" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (uq *UserQuery) loadSubscriptions(ctx context.Context, query *SubscribeQuery, nodes []*User, init func(*User), assign func(*User, *Subscribe)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(user.SubscriptionsTable)
		s.Join(joinT).On(s.C(subscribe.FieldID), joinT.C(user.SubscriptionsPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(user.SubscriptionsPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(user.SubscriptionsPrimaryKey[1]))
	...
	neighbors, err := withInterceptors[[]*Subscribe](ctx, query, qr, query.inters)
	if err != nil {
		return err
	}
	for _, n := range neighbors {
